...
public final class ParserAndTranslator {
  ...
  private static void whileStmt() throws SourceFileErrorException
  {
    ...
    accept(WHILE);
    accept(LPAREN);
    int loop_begin = Instruction.getNextCodeAddress();
    expr7();
    accept(RPAREN);
  ...
  private static void expr1() throws SourceFileErrorException
  {
    ...
    case UNSIGNEDINT:
	accept(UNSIGNEDINT);
	new PUSHNUMinstr(LexicalAnalyzer.getCurrentValue()); break;
    case NEW:
	accept(NEW);
	accept(INT);
	accept(LBRACKET);
